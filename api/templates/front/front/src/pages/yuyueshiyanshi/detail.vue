	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yuyueshiyanshi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.yonghuleixing}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yuyueshiyanshi','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yuyueshiyanshi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yuyueshiyanshi','取消')" 
							@click="onAcross('quxiaoyuyue','否','否','','')" type="warning">
							取消
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yuyueshiyanshi','签到')" 
							@click="onAcross('kaoqinjilu','是','否','','')" type="warning">
							签到
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('yuyueshiyanshi','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`yuyueshiyanshi`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yuyueshiyanshi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yuyueshiyanshiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此预约实验室？') .then(_ => {
					this.$http.post('yuyueshiyanshi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
