	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/shiyanshi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.shiyanshibianhao}}</div>
					</div>
					<div class="item">
						<div class="lable">实验室分类</div>
						<div class="text "  >{{detail.shiyanshifenlei}}</div>
					</div>
					<div class="item">
						<div class="lable">实验室规模</div>
						<div class="text "  >{{detail.shiyanshiguimo}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">安全等级</div>
						<div class="text "  >{{detail.anquandengji}}</div>
					</div>
					<div class="item">
						<div class="lable">实验室状态</div>
						<div class="text "  >{{detail.shiyanshizhuangtai}}</div>
					</div>
					<div class="item">
						<div class="lable">实验室位置</div>
						<div class="text "  >{{detail.shiyanshiweizhi}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shiyanshi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('shiyanshi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('shiyanshi','预约')" 
							@click="onAcross('yuyueshiyanshi','','','shiyanshizhuangtai','使用中','使用中,空闲中'.split(',')[0])" type="warning">
							预约
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="实验室详情" name="first">
					<div class="ql-snow ql-editor" v-html="detail.shiyanshixiangqing"></div>
				</el-tab-pane>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.shiyanshitupian) {
							this.detailBanner = this.detail.shiyanshitupian.split(",w").length>1?[this.detail.shiyanshitupian]:this.detail.shiyanshitupian.split(',');
						}
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('shiyanshi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'shiyanshi', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			getDiscussList(page) {
				this.$http.get('discussshiyanshi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussshiyanshi/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussshiyanshi/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussshiyanshi/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此实验室？') .then(_ => {
