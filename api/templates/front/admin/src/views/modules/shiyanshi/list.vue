<template>
	<div class="main-content" :style='{"padding":"20px 10px 20px 10px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"alignItems":"center","padding":"10px","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","width":"120px","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">实验室编号</label>
						<el-input v-model="searchForm.shiyanshibianhao" placeholder="实验室编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","width":"120px","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">实验室名称</label>
						<el-input v-model="searchForm.shiyanshimingcheng" placeholder="实验室名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="实验室分类" prop="shiyanshifenlei">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","width":"120px","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">实验室分类</label>
						<el-select clearable v-model="searchForm.shiyanshifenlei" placeholder="请选择实验室分类" >
							<el-option v-for="(item,index) in shiyanshifenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="实验室规模" prop="shiyanshiguimo">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","width":"120px","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">实验室规模</label>
						<el-select clearable v-model="searchForm.shiyanshiguimo" placeholder="请选择实验室规模" >
							<el-option v-for="(item,index) in shiyanshiguimoOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="安全等级" prop="anquandengji">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","width":"120px","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">安全等级</label>
						<el-select clearable v-model="searchForm.anquandengji" placeholder="请选择安全等级" >
							<el-option v-for="(item,index) in anquandengjiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="实验室状态" prop="shiyanshizhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","width":"120px","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">实验室状态</label>
						<el-select clearable v-model="searchForm.shiyanshizhuangtai" placeholder="请选择实验室状态" >
							<el-option v-for="(item,index) in shiyanshizhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"10px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('shiyanshi','使用率统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="shiyanshifenlei"
						label="实验室分类">
						<template slot-scope="scope">
							{{scope.row.shiyanshifenlei}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="shiyanshiguimo"
					...
					<el-table-column  :resizable='true' prop="shiyanshitupian" width="200" label="实验室图片">
						<template slot-scope="scope">
							<div v-if="scope.row.shiyanshitupian">
								<img v-if="scope.row.shiyanshitupian.substring(0,4)=='http'&&scope.row.shiyanshitupian.split(',w').length>1" :src="scope.row.shiyanshitupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.shiyanshitupian)">
								<img v-else-if="scope.row.shiyanshitupian.substring(0,4)=='http'" :src="scope.row.shiyanshitupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.shiyanshitupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.shiyanshitupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.shiyanshitupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('shiyanshi','预约')" @click="yuyueshiyanshiCrossAddOrUpdateHandler(scope.row,'cross','','','shiyanshizhuangtai','使用中','使用中,空闲中'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								预约
							</el-button>
		...
		<yuyueshiyanshi-cross-add-or-update v-if="yuyueshiyanshiCrossAddOrUpdateFlag" :parent="this" ref="yuyueshiyanshiCrossaddOrUpdate"></yuyueshiyanshi-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="shiyanshizhuangtaiChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import yuyueshiyanshiCrossAddOrUpdate from "../yuyueshiyanshi/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				yuyueshiyanshiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			yuyueshiyanshiCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			yuyueshiyanshiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yuyueshiyanshiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yuyueshiyanshiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yuyueshiyanshiCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var shiyanshizhuangtaiChart1 = echarts.init(document.getElementById("shiyanshizhuangtaiChart1"),'macarons');
					this.$http({
						url: "shiyanshi/group/shiyanshizhuangtai",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].shiyanshizhuangtai);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].shiyanshizhuangtai
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							shiyanshizhuangtaiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shiyanshizhuangtaiChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				if(this.searchForm.shiyanshifenlei!='' && this.searchForm.shiyanshifenlei!=undefined){
					params['shiyanshifenlei'] = this.searchForm.shiyanshifenlei
				}
				...
				if(this.searchForm.anquandengji!='' && this.searchForm.anquandengji!=undefined){
					params['anquandengji'] = this.searchForm.anquandengji
				}
