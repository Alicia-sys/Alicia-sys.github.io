<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('shiyanshi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{shiyanshiCount}}</div>
					<div class="name">实验室总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('yuyueshiyanshi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{yuyueshiyanshiCount}}</div>
					<div class="name">预约实验室总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 2 -->
		<div class="type2">
			<div id="shiyanshiChart1" class="echarts1 animate__animated" v-if="isAuth('shiyanshi','首页统计')"></div>
			<div id="yuyueshiyanshiChart1" class="echarts2 animate__animated" v-if="isAuth('yuyueshiyanshi','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getshiyanshiCount();
		if(this.isAuth('shiyanshi','首页统计')){
			this.shiyanshiChat1();
		}
		this.getyuyueshiyanshiCount();
		if(this.isAuth('yuyueshiyanshi','首页统计')){
			this.yuyueshiyanshiChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'shiyanshiChart1',css:'animate__fadeInUp'},
				{id:'yuyueshiyanshiChart1',css:'animate__fadeInUp'},
			]
...
// 1234 饼
		shiyanshiChat1() {
			this.$nextTick(()=>{
				...
				var shiyanshiChart1 = echarts.init(document.getElementById("shiyanshiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						shiyanshiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							shiyanshiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getyuyueshiyanshiCount() {
			this.$http({
				url: `yuyueshiyanshi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yuyueshiyanshiCount = data.data
				}
			})
		},
// 1234 竖
		yuyueshiyanshiChat1() {
			this.$nextTick(()=>{
				...
				var yuyueshiyanshiChart1 = echarts.init(document.getElementById("yuyueshiyanshiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						yuyueshiyanshiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							yuyueshiyanshiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
