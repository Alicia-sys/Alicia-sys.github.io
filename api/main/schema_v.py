...
@main_bp.route("/pythonek97261k/cal/<tableName>/<columnName>", methods=['GET'])
def pythonek97261k_cal(tableName, columnName):
    '''
    计算规则接口
    '''
...
@main_bp.route("/pythonek97261k/file/download", methods=['GET'])
def pythonek97261k_file_download():
    '''
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythonek97261k/file/upload", methods=['POST'])
def pythonek97261k_file_upload():
    '''
    '''
...
@main_bp.route("/pythonek97261k/follow/<tableName>/<columnName>/<level>/<parent>", methods=['GET'])
def pythonek97261k_follow_level(tableName, columnName, level,parent):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythonek97261k/follow/<tableName>/<columnName>", methods=['GET'])
def pythonek97261k_follow(tableName, columnName):
    '''
...
@main_bp.route("/pythonek97261k/location", methods=['GET'])
def pythonek97261k_location():
    '''
...
@main_bp.route("/pythonek97261k/matchFace", methods=['GET'])
def pythonek97261k_matchface():
    '''
    baidubce百度人脸识别
    '''
    if request.method == 'GET':
...
@main_bp.route("/pythonek97261k/option/<tableName>/<columnName>", methods=['GET'])
def pythonek97261k_option(tableName, columnName):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythonek97261k/remind/<tableName>/<columnName>/<type>", methods=['GET'])  #
def pythonek97261k_remind(tableName, columnName,type):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/pythonek97261k/sh/<tableName>", methods=['POST'])
def pythonek97261k_sh(tableName):
    '''
    '''
    if request.method == 'POST':
        ...
        logging.warning("pythonek97261k_sh.res=====>{}".format(res))
        return jsonify(msg)

@main_bp.route("/pythonek97261k/upload/<path:filepath>", methods=['GET'])
def pythonek97261k_upload(filepath):
    '''
    上传接口
    '''
    if request.method == 'GET':
        ...
        if len(file_list)>=2:
            return redirect(f"/{file_list[0]}/{file_list[1]}")
        else:
            return redirect(f"/{filepath}")

@main_bp.route("/pythonek97261k/group/<tableName>/<columnName>", methods=['GET'])
...
@main_bp.route("/pythonek97261k/value/<tableName>/<xColumnName>/<yColumnName>", methods=['GET'])
...
# 数据备份
@main_bp.route("/pythonek97261k/mysqldump", methods=['GET'])
def schema_mysqldump():
    if request.method == 'GET':
        filename = "mysql.dmp"
        filepath = '{}/api/templates/front'.format(os.getcwd())
        cmd = '"C:\\Program Files\\MySQL\\MySQL Server 5.7\\bin\\mysqldump" -h127.0.0.1 -uroot -P3306 -p123456 pythonek97261k > ' + filepath + "/" + filename
        os.system(cmd)
        return send_from_directory(filepath, filename, as_attachment=True)
